	...
	def startStream(self):
		if not self.container:
			...
			if len(filters) == 0:
				filters.append("null")
			try:
				self.container = self.client.containers.run("adilinden/rpi-stream:latest",command=["octopi-youtubelive",self._settings.get(["webcam_url"]),self._settings.get(["stream_url"]),self._settings.get(["stream_id"]),",".join(filters)],detach=True,privileged=True,name="YouTubeLive",auto_remove=True)
				self._plugin_manager.send_plugin_message(self._identifier, dict(status=True,streaming=True))
			except Exception, e:
